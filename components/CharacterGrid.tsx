...
const CharacterDetail: React.FC<{ character: Character; onClose: () => void }> = ({ character, onClose }) => {
  ...
  return (
    <motion.div
      className={`fixed inset-0 z-[60] flex items-center justify-center backdrop-blur-xl ${isGear5 ? 'bg-white/95' : 'bg-black/95'}`}
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
    ...
    >
        ...
        <div className={`relative w-full md:w-1/2 h-1/2 md:h-full flex flex-col justify-center p-8 md:p-20 overflow-y-auto ${isGear5 ? 'bg-white' : `bg-gradient-to-br ${character.bgGradient || 'from-slate-900 to-slate-950'}`}`}>
           ...
           >
             ...
             <h2 className={`text-5xl md:text-8xl font-black font-serif mb-2 uppercase tracking-tighter leading-[0.8] ${isGear5 ? 'text-sky-900' : 'text-white'}`}>
               {character.name.split(' ').map((word, i) => <span key={i} className="block">{word}</span>)}
             </h2>
             <p className={`${isGear5 ? 'text-sky-600' : 'text-white/40'} text-xl font-serif italic mb-8 tracking-wider`}>{character.epithet}</p>

             <div className={`border-l-4 pl-6 py-2 mb-10 ${isGear5 ? 'border-sky-400 bg-gradient-to-r from-sky-50 to-transparent' : 'border-amber-500 bg-gradient-to-r from-amber-500/10 to-transparent'}`}>
                <p className={`text-2xl md:text-3xl font-serif italic leading-tight ${isGear5 ? 'text-slate-700' : 'text-slate-200'}`}>
                  "{character.quote}"
                </p>
             </div>

             <p className={`text-lg leading-relaxed mb-10 max-w-lg ${isGear5 ? 'text-slate-600' : 'text-slate-400'}`}>
               {character.description}
             </p>

             <div className={`inline-flex items-center gap-4 px-6 py-4 rounded-lg border ${isGear5 ? 'bg-white border-sky-100 shadow-lg' : 'bg-white/5 border-white/10'}`}>
               <span className={`uppercase tracking-widest text-xs ${isGear5 ? 'text-slate-500' : 'text-white/40'}`}>Current Bounty</span>
               <span className={`font-mono text-2xl md:text-3xl font-bold ${isGear5 ? 'text-sky-600' : 'text-amber-400'}`}>{character.bounty}</span>
             </div>
    ...
    <LayoutGroup>
      <section id="wanted" ref={targetRef} className={`relative h-[350vh] transition-colors duration-1000 ${isGear5 ? 'bg-white' : 'bg-[#080808]'}`}>
        <div className="sticky top-0 flex h-screen items-center overflow-hidden">
          ...
          >
             <h2 className={`font-serif text-4xl md:text-8xl font-black mb-2 tracking-tight uppercase shadow-black drop-shadow-2xl ${isGear5 ? 'text-sky-900' : 'text-white'}`}>Wanted</h2>
             <p className={`font-serif italic text-lg md:text-2xl tracking-widest drop-shadow-lg ${isGear5 ? 'text-sky-500' : 'text-amber-500'}`}>Dead or Alive</p>
             <p className={`mt-2 text-xs tracking-[0.3em] uppercase ${isGear5 ? 'text-slate-400' : 'text-white/50'}`}>Straw Hat Pirates</p>
          </motion.div>
          ...
          <motion.div style={{ x }} className="flex gap-12 pl-[40vw] pr-20 items-center relative z-10">
            {CHARACTERS.map((char) => (
              <motion.div
                ...
                className={`group relative h-[60vh] w-[80vw] md:w-[30vw] flex-shrink-0 cursor-pointer overflow-hidden rounded-xl border grayscale transition-all duration-500 hover:grayscale-0 perspective-1000 shadow-2xl ${isGear5 ? 'bg-white border-sky-100 hover:border-sky-400 shadow-sky-900/10' : 'bg-slate-900 border-white/5 hover:border-amber-500/50 shadow-black/50'}`}
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 w-full p-8 translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                  <p className={`text-xs font-bold tracking-[0.2em] uppercase mb-2 opacity-0 group-hover:opacity-100 transition-opacity delay-100 ${isGear5 ? 'text-sky-600' : 'text-amber-500'}`}>
                    {char.role}
                  </p>
                  <h3 className={`font-serif text-4xl font-black leading-none mb-2 uppercase tracking-tighter drop-shadow-md ${isGear5 ? 'text-sky-900' : 'text-white'}`}>
                    {char.name}
                  </h3>
                  <p className={`text-sm italic mb-4 font-serif ${isGear5 ? 'text-slate-500' : 'text-slate-400'}`}>{char.epithet}</p>

                  <div className={`flex items-center gap-3 border-t pt-4 mt-4 ${isGear5 ? 'border-sky-900/10' : 'border-white/10'}`}>
                    <span className={`text-xs tracking-widest uppercase ${isGear5 ? 'text-slate-400' : 'text-white/50'}`}>Bounty</span>
                    <div className={`font-mono text-xl font-bold tracking-widest ${isGear5 ? 'text-sky-600' : 'text-amber-500'}`}>
                      {char.bounty}
                    </div>
                  </div>
                </div>
          ...
          {/* Progress Bar */}
          <div className={`absolute bottom-10 left-10 right-10 h-1 rounded-full overflow-hidden z-20 ${isGear5 ? 'bg-sky-100' : 'bg-white/10'}`}>
             <motion.div
